...
1. **回复标签要求**: 你的回复必须严格遵守输出格式要求，格式要求完整，始终包含`<thinking></thinking>`以说明推理过程，同时必须提供`<tool_call></tool_call>`或`<response></response>`中的一项。
2. **工具调用要求**: 你可以在`<tool_call></tool_call>`中同时调用多个工具。每个工具调用应是一个包含"name"字段和"arguments"字段的JSON对象，其中arguments字段包含参数字典。若无需参数，则将arguments字段留空字典。
...
9. **信息完整性要求**: 发现信息缺失，并且评估可以使用工具查询时，请立即使用`<tool_call></tool_call>`调用工具，将信息补充完整。
