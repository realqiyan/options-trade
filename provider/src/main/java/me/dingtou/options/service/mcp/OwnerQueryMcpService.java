...
public class OwnerQueryMcpService {
    ...
    // public String queryPositionByCode(@ToolParam(required = true, description =
    // "用户Token") String ownerCode,
    // @ToolParam(required = true, description = "股票或期权代码，如'BABA'、'AAPL'等") String
    // code,
    // @ToolParam(required = true, description = "市场代码：1表示港股，11表示美股") Integer
    // market) {
    // String owner = authService.decodeOwner(ownerCode);
    // if (null == owner) {
    // return "用户编码信息不正确或已经过期";
    ...
    // List<OwnerPosition> ownerPositionList =
    // tradeManager.queryOwnerPosition(account, List.of(code));
    // // 准备模板数据
    // Map<String, Object> data = new HashMap<>();
    ...
    // public String queryAllPosition(@ToolParam(required = true, description =
    // "用户Token") String ownerCode) {
    // String owner = authService.decodeOwner(ownerCode);
    // if (null == owner) {
    // return "用户编码信息不正确或已经过期";
    ...
    // OwnerAccount account = ownerManager.queryOwnerAccount(owner);
    // List<OwnerPosition> ownerPositionList =
    // tradeManager.queryOwnerPosition(account, null);
