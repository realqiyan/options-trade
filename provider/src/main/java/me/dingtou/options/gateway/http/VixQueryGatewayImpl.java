...
public class VixQueryGatewayImpl implements VixQueryGateway {
    ...
    private static final String URL = "https://www.spglobal.com/spdji/en/util/redesign/get-index-comparison-data.dot?compareArray=92026376&compareArray=340&periodFlag=monthToDateFlag&language_id=142";
    ...
    public VixIndicator queryCurrentVix() {
        ...
        try {
            ...
            if (response.code() != 200) {
                // 重试一次
                log.warn("查询VIX失败,重试一次,statusCode:{}", response.code());
                response = CLIENT.newCall(request).execute();
            }
            if (response.code() == 200) {
                VixIndicator indicator = convert(response.body().string());
                VIX_CACHE.put("vixIndicator", indicator);
                return indicator;
            }
            log.error("查询VIX失败,statusCode:{}", response.code());
        } catch (Exception e) {
